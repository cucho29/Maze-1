...
class Controller:
    ...
    ***************************
    ...
    def read_a_client(self):
        ...
        id_cliente = input()
        client = self.model.read_a_client(id_cliente)
    ...
    ***************************
    ...
    def create_order(self):
        ...
        if type(id_order) == int:
            ...
            while id_product != '':
                ...
                id_product, od_total = self.create_order_detail(id_order)
                o_total += od_total
    ...
    def read_a_order(self):
        ...
        if type(order) == tuple:
            order_details = self.model.read_a_order_detail(id_order)
            if type(order_details) != list and order_details != None:
                ...
                self.view.show_a_order_details_header()
                for order_detail in order_details:
                    self.view.show_a_order_details(order_detail)
                self.view.show_a_order_details_footer()
    ...
    def read_orders_client(self):
        self.view.ask('ID Cliente: ')
        id_cliente = input()
        orders = self.model.read_orders_client(id_cliente)
        if type(orders) == list:
            self.view.show_order_header( 'Ordenes para el cliente '+id_cliente+' ')
            for order in orders:
                id_order = order[0]
                order_details = self.model.read_a_order_detail(id_order)
                if type(order_details) != list and order_details != None:
            ...
            self.view.error('PROBLEMA AL LEER LAs VENTAa. REVISA.')
        return
    ...
    def update_order(self):
        ...
        o_date = input()
        whole_vals = [id_cliente, o_status, o_date]
        fields, vals = self.update_lists(['id_cliente', 'o_status', 'o_date'], whole_vals)
    ...
    """
    *******************************
    ** Controlador Detalle_Ventas *
    *******************************
    """ 
    def create_order_detail(self, id_order):
        od_total = 0.0
        self.view.ask('ID Producto: ')
        id_product = input()
        if id_product != '':
    ...
    def delete_order_detail(self):
        ...
        if type(order) == tuple:
            ...
            id_product = ' '
            while id_product != '':
                self.view.msg('---- Borra productos de la orden (deja vacio el ID del producto para salir)----')
                self.view.ask('ID Producto: ')
                id_product = input()
                if id_product != '':
                    order_detail = self.model.read_a_order_detail(id_order, id_product)
